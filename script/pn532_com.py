...
class Pn532Com:
    ...
    def is_pn532killer(self):
        response = self.send_cmd_sync(Command.checkPn532Killer)
            ...
            if len(data_bytes) > 0:
                ...
                if data_position == 0:
                    if len(data_buffer) < 1:
                        print("Waiting for more bytes at position 0")  # Wait for more bytes
                    ...
                    if len(data_buffer) < 2:
                        print("Waiting for more bytes at position 1")  # Wait for more bytes
                    # if data_buffer[1] != 0x00:
                    #     print("Data frame start code error.")
                    #     continue
                elif data_position == 2:
                    if len(data_buffer) < 3:
                        print("Waiting for more bytes at position 2")  # Wait for more bytes
                    # if data_buffer[2] != 0xFF:
                    #     print("Data frame start code error.")
                    #     continue
                elif data_position == 3:
                    if len(data_buffer) < 4:
                        print("Waiting for more bytes at position 3")  # Wait for more bytes
                    ...
                    # Check DCS
                    if data_buffer[4 + data_length + 1] != self.dcs(data_buffer[5 : 4 + data_length + 1]):
                    ...
                    if data_cmd in self.wait_response_map:
                        ...
                        if(data_cmd == Command.InCommunicateThru and len(data_response) > 2):
                            response = Response(data_cmd, data_response[2], data_response[2:])
                            if(data_response[2] == 0 and len(data_response) > 16):
                                response = Response(data_cmd, data_response[2], data_response[3:data_length - 2])
    ...
    def send_cmd_auto(self, cmd: int, data: Union[bytes, None] = None, status: int = 0, callback=None, timeout: int = 1,
                      close: bool = False):
        """
            Send cmd to device
