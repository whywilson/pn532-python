...
static uint32_t atoui(const char *s) {
    // Accept hex (with or without 0x) or decimal for convenience.
    int base = 10;
    const char *p = s;
    ...
    return (uint32_t)strtoul(p, NULL, base);
}
...
int main(int argc, char *const argv[]) {
    ...
    if (argc < 5 || ((argc - 3) % 2) != 0) {
        fprintf(stderr, "usage: %s <uid> <keytype> <nt1> <par1> [<nt2> <par2> ...]\n", argv[0]);
        fprintf(stderr, "  values can be decimal or hex (0x...); keytype: 0x60/96 for KeyA, 0x61/97 for KeyB\n");
        return EXIT_FAILURE;
    }
    ...
    for (i = 3, j = 0; i < argc; i += 2) {
        ...
        if (!check_st_level_at_first_run) {
            if (nt1 == 0x01200145) {
                ...
                fprintf(stderr, "Unsupported static tag nonce 0x%08X (expected 0x01200145 gen1 or 0x009080A2 gen2)\n", nt1);
                goto error;
            }
...
error:
    free(pNK);
    fprintf(stderr, "staticnested failed (bad input or allocation error)\n");
    exit(EXIT_FAILURE);
}
